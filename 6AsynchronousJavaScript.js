...
A modern approach to handle asynchronous code.
States:
1. Pending: Initial state.
2. Fulfilled: Resolved successfully.
3. Rejected: Encountered an error.
*/
...
/* Async/Await:
- A modern syntax for handling asynchronous code.
...
const data = await new Promise((resolve ,) =>
  setTimeout(() => resolve("Fetched data new promise"), 1000)
);

async function fetchData() {
  const datamain = await data;
